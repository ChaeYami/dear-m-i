...
public class PrescriptionMedDrugInfoService {
    ...
    public void refreshAsync(UUID medicationId, UUID userId) {
        ...
        var prescription = prescriptionRepository
                .findByIdAndUserIdAndDeletedAtIsNull(med.getPrescriptionId(), userId).orElse(null);
        ...
        try {
            Optional<DrugInfoDto> info = drugInfoRouter.searchByName(med.getDrugName(), prescription.getRegion());
            if (info.isPresent()) {
